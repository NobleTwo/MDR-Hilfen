  ...
  public void bezieheZulassungenEin(){
    for(int disziplin=0; disziplin<wettbewerbschancen.length; disziplin++){
      ...
      for (int niveau = wettbewerbschancen[0].length-1; niveau>=0; niveau--) {
        ...
        /*boolean[] zulassungen = new boolean[anzahlWettbewerbsniveaus];
        for(int überprüfterWert=0; überprüfterWert<p.anzahlPferdezahlen; überprüfterWert++){
        if (db.disziplinen[disziplin][überprüfterWert]==true) { 
        for (int k=0; k<anzahlWettbewerbsniveaus; k++) {
        if (alleWerte[überprüfterWert]>grenzen[k-1] && alleWerte[überprüfterWert]<grenzen[k]+1) {
        zulassungen[k]=true;
        break;
        } 
        }
        ...
        for(int j=0; j<anzahlWettbewerbsniveaus; j++){
        if (zulassungen[j]==true) {
        wettbewerbschancen[disziplin][j]=0;
        } 
        else{
        wettbewerbschancen[disziplin][j]=-100;
        } */
      }
    }
  }   
